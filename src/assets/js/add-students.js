    ...
    async function addNewGrade() {
        const gradeNumberInput = document.getElementById('newGradeNumber');
        const gradeStrandInput = document.getElementById('newGradeStrand');
        const submitBtn = event.target; 
        
        const gradeNumber = gradeNumberInput.value.trim();
        const strand = gradeStrandInput.value.trim();
        
        if (!gradeNumber) {
            showModal('error', 'Empty Grade', 'Please enter a Grade Level number.');
            gradeNumberInput.focus();
            return;
        }

        let name = `Grade ${gradeNumber}`;
        if (strand) {
            name += ` - ${strand}`;
        }
        ...
        gradeNumberInput.readOnly = true;
        gradeStrandInput.readOnly = true;
        ...
        try {
            ...
            if (res.success) {
                ...
                gradeNumberInput.readOnly = false;
                gradeStrandInput.readOnly = false;
            }
        } catch (err) {
